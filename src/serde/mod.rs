...
//! Serde (de)serialization for [`crate::Cid`]
mod de;
mod ser;
...
mod tests {
    ...
    fn test_cidv1_serde_cbor() {
        ...
        let v = multi_cbor::to_vec(&v1).unwrap();
        assert_eq!(
            v,
            hex::decode(
        ...
        assert_eq!(v1, multi_cbor::from_slice(&v).unwrap());
    }
    ...
    #[cfg(feature = "dag_cbor")]
    #[test]
    fn test_null_cid_serde_cbor() {
        let c = cid::Cid::null();
        // With dag_cbor feature, CIDs are serialized as CBOR tag 42 with the CID bytes
        let v = multi_cbor::to_vec(&c).unwrap();
        // d82a = CBOR tag 42, 45 = bytes(5), 00 = identity codec, 01000000 = null CID bytes
        assert_eq!(v, hex::decode("d82a450001000000").unwrap());
        assert_eq!(c, multi_cbor::from_slice(&v).unwrap());
    }
